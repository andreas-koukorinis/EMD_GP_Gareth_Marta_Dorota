...
n <- 200     #lenght time-series  500 -1000 - 5000 (subset of 5000 to subsample - adding additional observ.)
m <-1000       #number of time-series     10000
t<- vector(mode = "list", m)
t <- lapply(1:m, function(i) t[[i]]<- c(0,sort(runif(n-2,0,10)),10) ) #equidistant seq(0,runif(1,0,10), len = n))
#(0,1) convert into 0,1 interval
...
K_SE <- function(x1,x2,l = 1) {exp(- abs(x1-x2)^2 / (2 * l^2) ) }  #sigma^2
...
K_OLS <- function(x1,x2,sigma2 = 1){x1 * x2 + sigma2} #VECTOR inputs for all of the kernels

#add decaying periodic - gareth notes
#linear mixture kernel
#matern kernel
#anova/bestsubest kernel
#(p-spline polynomial kernel)
#(neural network kernel - GP book)

#GRID EXPANSIONS ---------------------------------------------------------------------------------
#1) SQUARE EXPONENATIAL
l_list =  c(0.25,1,3,5,7,0.25,1,3,5,7)   #.25, 0.5 , 0.75, 1, 1.5, 2 - gareth discussion   - plot for each kernel         
nl = length(l_list)
